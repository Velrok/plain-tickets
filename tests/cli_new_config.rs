...
fn new_uses_config_default_type() {
    ...
    let filename = String::from_utf8_lossy(&out.stdout)
        .trim()
        .splitn(2, ' ')
        .nth(1)
        .unwrap()
...
fn init_scaffolds_new_section_in_config() {
    ...
    assert!(
        ...
        "expected [new] section mention: {}",
        content
    );
    assert!(
        content.contains("default_status") || content.contains("default_type"),
        "expected default_status or default_type mention: {}",
        content
    );
}
